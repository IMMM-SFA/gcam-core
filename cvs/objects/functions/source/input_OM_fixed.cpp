...
{   
    ...
    double levelizedOMFixedCost = calcLevelizedOMFixedCost( capacityFactor );
    ...
    // penalties.
    fill( mAdjustedCosts.begin(), mAdjustedCosts.end(), levelizedOMFixedCost );
}
 ...
 * \author Sonny Kim
 */
double InputOMFixed::calcLevelizedOMFixedCost( const double aCapacityFactor ) const
{
    // TODO: Use technology's capacity factor.
    // TODO: Use Value class for units conversion.
    double levelizedOMFixedCost = mOMFixed 
        / ( FunctionUtils::HOURS_PER_YEAR() * aCapacityFactor * FunctionUtils::GJ_PER_KWH() );
