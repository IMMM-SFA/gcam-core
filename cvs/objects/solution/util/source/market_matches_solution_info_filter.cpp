...
bool MarketMatchesSolutionInfoFilter::XMLParse( const DOMNode* aNode ) {
    ...
    for ( unsigned int i = 0; i < nodeList->getLength(); ++i ){
        ...
        else if( nodeName == "market-matches" ) {
            string matchStr = XMLHelper<string>::getValue( curr );
            regex matchRegex(matchStr, regex::nosubs | regex::optimize | regex::egrep);
            mAcceptMarketMatches.swap(matchRegex);
        }
        else {
