...
module_gcamusa_batch_nonewcoal_USA_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2231.SubsectorShrwt_elec_nonewcoal_USA <- get_data(all_data, "L2231.SubsectorShrwt_elec_nonewcoal_USA")
    ...
    create_xml("nonewcoal_USA.xml") %>%
      ...
      add_xml_data_generate_levels(L2231.SubsectorShrwt_elec_nonewcoal_USA, "SubsectorShrwt",
                                   "subsector", "nesting-subsector", 1, FALSE) %>%
      add_precursors("L2231.SubsectorShrwt_elec_nonewcoal_USA",
                     "L2231.StubTechShrwt_nonewcoal_USA") ->
      nonewcoal_USA.xml
