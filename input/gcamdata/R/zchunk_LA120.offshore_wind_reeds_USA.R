...
module_gcamusa_LA120.offshore_wind_reeds_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    reeds_offshore_wind_curve_grid_cost %>%
      ...
      ungroup() -> L120.GridCost_offshore_wind_USA
    ...
    L120.RenewRsrc_offshorewind_reeds_USA %>%
      add_title("Offshore wind resource") %>%
      add_units("NA") %>%
      add_comments("Offshore wind resource tags by states") %>%
      same_precursors_as(L120.GrdRenewRsrcCurves_offshorewind_reeds_USA) ->
      L120.RenewRsrc_offshorewind_reeds_USA
    ...
    return_data(L120.GrdRenewRsrcCurves_offshorewind_reeds_USA, L120.GridCost_offshore_wind_USA, L120.RenewRsrc_offshorewind_reeds_USA)
  } else {
