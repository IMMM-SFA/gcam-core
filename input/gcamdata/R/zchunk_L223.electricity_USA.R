...
#' \code{L223.SubsectorLogit_Investment}, \code{L223.SubsectorShrwt_Investment},\code{L223.StubTech_elec_USA},
...
module_gcamusa_L223.electricity_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A23.globaltech_co2capture <- get_data(all_data, "energy/A23.globaltech_co2capture", strip_attributes = TRUE)
    ...
    A23.elec_tech_mapping_cool_shares_fut <- get_data(all_data, "gcam-usa/A23.elec_tech_mapping_cool_shares_fut", strip_attributes = TRUE)
    ...
    L114.CapacityFactor_wind_state <- get_data(all_data, "L114.CapacityFactor_wind_state_gcamusa", strip_attributes = TRUE)
    ...
    A23.dispatch_subsector_shrwt_state_adj <- get_data(all_data, "gcam-usa/A23.dispatch_subsector_shrwt_state_adj", strip_attributes = TRUE)
    ...
    A23.dispatch_globaltech_eff_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_eff_additional", strip_attributes = TRUE)
    A23.dispatch_globaltech_OMfixed_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_OMfixed_additional", strip_attributes = TRUE)
    A23.dispatch_globaltech_OMvar_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_OMvar_additional", strip_attributes = TRUE)
    A23.dispatch_globaltech_capital_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_capital_additional", strip_attributes = TRUE)
    A23.dispatch_globaltech_retirement_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_retirement_additional", strip_attributes = TRUE)
    A23.dispatch_globaltech_shrwt_additional <- get_data(all_data, "gcam-usa/A23.dispatch_globaltech_shrwt_additional", strip_attributes = TRUE)
    A23.globaltech_keyword_additional <- get_data(all_data, "gcam-usa/A23.globaltech_keyword_additional", strip_attributes = TRUE)
    A23.dispatch_additional_mapping <- get_data(all_data, "gcam-usa/A23.dispatch_additional_mapping", strip_attributes = TRUE)
    ...
    capacity_credit_calculator <- get_data(all_data, "gcam-usa/dispatch/capacity_credit_calculator", strip_attributes = TRUE)
    TechTrialMarket_mapping <- get_data(all_data, "gcam-usa/dispatch/TechTrialMarket_mapping", strip_attributes = TRUE)
    ...
    L210.GrdRenewRsrcCurves_geo_USA <- get_data(all_data, "L210.GrdRenewRsrcCurves_geo_USA", strip_attributes = TRUE)
    L120.GridCost_offshore_wind_USA <- get_data(all_data, "L120.GridCost_offshore_wind_USA", strip_attributes = TRUE)
    ...
    process_USA_to_states(L223.StubTech_elec) %>%
      ...
      anti_join(A10.renewable_resource_delete,
                by = c("region", "subsector" = "resource_elec_subsector"))-> L223.StubTech_elec_USA

    # Modifications for offshore wind
    # Remove states with no offshore wind resources
    offshore_wind_states <- L210.GrdRenewRsrcCurves_geo_USA %>% filter(renewresource == "offshore wind resource") %>% pull(region) %>% unique()
    ...
    L223.SubsectorShrwt_Investment_Fuel_base %>%
      ...
      # geothermal is causing solution issues... set shareweight to 0 to help solution
      # TODO: think about a better approach for geothermal
      mutate(share.weight = ifelse(subsector == "geothermal", 0, share.weight)) %>%
      # 2. For nuclear - don't allow new investment (use zero  shareweights) if there is no capacity historically
      # L123.capacity_EJ_state_elec_F_tech_final_cal_year does not include fuels with zero capacity in a state historically
      # thus it is missing entries - mostly for nuclear, wind, and solar - relative to LHS
      # LJENM will throw error because of NAs, left_join is used, and NAs are resolved below
      left_join(L123.capacity_EJ_state_elec_F_tech_final_cal_year, by = c("region", "subsector")) %>%
    ...
    return_data(L223.StubTech_elec_USA,
                L223.Sector_Investment,
                L223.SubsectorLogit_Investment_Fuel,
                L223.SubsectorShrwt_Investment_Fuel,
                L223.SubsectorInterpTo_Investment_Fuel,
