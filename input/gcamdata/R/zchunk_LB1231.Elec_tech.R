...
module_gcamusa_LB1231.Elec_tech<- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1231.in_EJ_R_elec_F_tech_Yh <- get_data(all_data, "L1231.in_EJ_R_elec_F_tech_Yh", strip_attributes=TRUE)
    ...
    L123.out_EJ_state_elec_F <- get_data(all_data, "L123.out_EJ_state_elec_F", strip_attributes=TRUE)
    ...
    # Expand generation in to include all fuels
    L123.in_EJ_state_elec_F %>%
      complete(fuel,nesting(state,sector,year)) %>%
      replace_na(list(value = 0))-> L123.in_EJ_state_elec_F_expand
    ...
    L1231.in_EJ_state_elec_F_tech <- L1231.share_elec_F_tech %>%
      ...
      left_join_error_no_match(L123.in_EJ_state_elec_F_expand, by = c("state", "sector", "fuel", "year")) %>%
    ...
    L123.out_EJ_state_elec_F %>%
      complete(fuel,nesting(state,sector,year)) %>%
      replace_na(list(value = 0))-> L123.out_EJ_state_elec_F_expand

    L1231.out_EJ_state_elec_F_tech <- L1231.share_elec_F_tech %>%
      left_join_error_no_match(L123.out_EJ_state_elec_F_expand, by = c("state", "sector", "fuel", "year")) %>%
      # State/Technology output = technology share * state/fuel output
      mutate(value = value.x * value.y) %>%
      select(-value.x, - value.y)
