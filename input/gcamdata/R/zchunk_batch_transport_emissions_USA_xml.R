...
module_gcamusa_batch_transport_emissions_USA_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L271.nonco2_trn_emiss_coeff_USA <- get_data(all_data, "L271.nonco2_trn_emiss_coeff_USA")

    # ===================================================

     # Produce outputs
    create_xml("transport_emissions_USA.xml") %>%
      add_xml_data(L271.nonco2_trn_tech_coeff_USA %>% rename(subsector = tranSubsector), "InputEmissions") %>%
      add_xml_data(L271.nonco2_trn_emiss_coeff_USA %>% rename(subsector = tranSubsector),"InputEmissCoeff") %>%
