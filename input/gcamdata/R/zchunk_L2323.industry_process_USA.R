#' module_gcamusa_L2323.industry_process_USA
#'
...
module_gcamusa_L2323.industry_process_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("L231.IndCoef",
    ...
    L232.StubTechCoef_industry_USA %>%
      ...
      left_join(L231.IndCoef_USA %>%
                  select(-region),
                by = c("supplysector", "subsector", "stub.technology" = "technology", "year")) %>%
      mutate(market.name = gcamusa.DEFAULT_MARKET,
             coefficient = round(coefficient, emissions.DIGITS_EMISSIONS)) -> L2323.StubTechCoef_indproc_USA
    ...
    L2323.StubTechCoef_indproc_USA %>%
      ...
      add_units("unitless") %>%
      add_comments("Industrial processes coefficients are from L231.IndCoef.") %>%
